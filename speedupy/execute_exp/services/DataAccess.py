# Arquivo: speedupy/execute_exp/DataAccess.py
...
class DataAccess(metaclass=SingletonMeta):
    def __init__(self):
        ...
        self.__function_calls_prov_table:FunctionCallProvTable
    ...
    def get_cache_entry(self, func_qualname, func_args, func_kwargs):
        ...
        entry = None
        if SpeeduPySettings().monitor_cache:
            start = time.perf_counter()
            entry = self.__mem_arch.get_cache_entry(func_call_hash, func_qualname)
            duration = time.perf_counter() - start
            if entry is not None:
                monitoring.cache_hits += 1
                monitoring.hit_total_time += duration
            else:
