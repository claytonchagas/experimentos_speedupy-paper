...
def analyse(spin=PARTICLE_SPIN, PARAM=0):
    ...
    abdeg = val(tuple(numpy.degrees(ab)))
    adeg = val(tuple(numpy.degrees(alice[:, 0])))
    bdeg = val(tuple(numpy.degrees(bob[:, 0])))
    # Convertendo numpy arrays para tuplas
    abdeg_tuple = tuple(abdeg)
    adeg_tuple = tuple(adeg)
    bdeg_tuple = tuple(bdeg)
    alice_tuple = tuple(map(tuple, alice))
    bob_tuple = tuple(map(tuple, bob))
    
    angles, o_angles, Eab, Corr = find_all_settings_used_in_simulation(abdeg_tuple, adeg_tuple, bdeg_tuple, alice_tuple, bob_tuple)
    
    # Convertendo pra tuplas
    angles_tuple = tuple(angles)
    o_angles_tuple = tuple(o_angles)
    Eab_tuple = tuple(Eab)
    Corr_tuple = tuple(map(tuple, Corr))
    
    display_results(abdeg_tuple, adeg_tuple, bdeg_tuple, alice_tuple, bob_tuple, angles_tuple, o_angles_tuple, Eab_tuple, Corr_tuple, spin, PARAM)

@cache
def find_all_settings_used_in_simulation(abdeg, adeg, bdeg, alice, bob):
    #convertendo de volta pra numpy arrays
    adeg = numpy.array(adeg)
    bdeg = numpy.array(bdeg)
    alice = numpy.array(alice)
    bob = numpy.array(bob)
...
def display_results(abdeg, adeg, bdeg, alice, bob, angles, o_angles, Eab, Corr, spin, PARAM):
    # Convertendo de volta pra numpy array
    abdeg = numpy.array(abdeg)
    adeg = numpy.array(adeg)
    bdeg = numpy.array(bdeg)
    alice = numpy.array(alice)
    bob = numpy.array(bob)
    ...
    ax1.plot(angles, QMFunc(tuple(numpy.radians(angles)), spin), 'b-+', label='QM', lw=0.5)
    bx, by = BellFunc(spin)
    ax1.plot(bx, by, 'r--', label='Bell', lw=1)
    ax1.legend()
    ax1.set_xlim(0, 360)
    ax2 = fig.add_subplot(122, projection='3d')
    # Convert Corr back to a numpy array for plotting
    Corr_array = numpy.array(Corr)
    ax2.plot_surface(X, Y, Corr_array, rstride=1, cstride=1, cmap=cm.coolwarm)
